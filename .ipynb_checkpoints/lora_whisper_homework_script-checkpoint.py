...
if args.mode == 'test':
    ...
    small_common_voice["train"] = common_voice["train"].shuffle(seed=16).select(range(64))
    small_common_voice["validation"] = common_voice["validation"].shuffle(seed=16).select(range(32))
    tokenized_common_voice = small_common_voice.map(prepare_dataset)
else:
    # 完整数据训练，尝试开启 `num_proc=8` 参数多进程并行处理（如阻塞无法运行，则不使用此参数）
    tokenized_common_voice = common_voice.map(prepare_dataset, num_proc=12)
...
print(f'check out result: tensorboard --logdir {logging_dir}')
